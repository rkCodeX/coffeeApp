        ...
        <>
            <div className='w-[100%] h-auto px-[4rem] py-[6rem] flex flex-col gap-[2.5rem]'>
                ...
                <div className='flex flex-row items-center justify-center w-[100%] h-auto'>
                    <button onClick={()=> {handleClick(); setBtn1(true) set}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn1? 'bg-transparent' : 'bg-red-950'}  border-red-950 border-2`}></button>
                    <button onClick={()=> {handleClick(); setBtn2(true) set}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn2? 'bg-transparent' : 'bg-red-950'}  border-red-950 border-2`}></button>
                    <button onClick={()=> {handleClick(); setBtn3(true) set}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn3? 'bg-transparent' : 'bg-red-950'}  border-red-950 border-2`}></button>
                    <button onClick={()=> {handleClick(); setBtn4(true) set}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn4? 'bg-transparent' : 'bg-red-950'}  border-red-950 border-2`}></button>
                </div>
            </div>
        </>
    );
