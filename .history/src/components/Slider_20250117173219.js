        ...
        <>
            <div className='w-[100%] h-auto px-[4rem] py-[6rem] flex flex-col gap-[2.5rem]'>
                ...
                <div className='flex flex-row items-center justify-center w-[100%] h-auto'>
                    <button onClick={()=> {handleClick1(); setBtn1(true); setBtn2(false); setBtn3(false); setBtn4(false)}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn1? 'bg-transparent' : 'bg-red-95'}  border-red-950 border-2`}></button>
                    <button onClick={()=> {handleClick2(); setBtn1(false); setBtn2(true); setBtn3(false); setBtn4(false)}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn2? 'bg-transparent' : 'bg-red-95'}  border-red-950 border-2`}></button>
                    <button onClick={()=> {handleClick3(); setBtn1(false); setBtn2(false); setBtn3(true); setBtn4(false)}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn3? 'bg-transparent' : 'bg-red-95'}  border-red-950 border-2`}></button>
                    <button onClick={()=> {handleClick4(); setBtn1(false); setBtn2(false); setBtn3(false); setBtn4(true)}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn4? 'bg-transparent' : 'bg-red-95'}  border-red-950 border-2`}></button>
                </div>
            </div>
        </>
    );
