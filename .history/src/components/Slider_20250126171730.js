...
function Slider() {
    ...
    const [ItemsPerPage, setItemsPerPa] = useState(4); 
        ...
        <>
            <div className='w-[100%] h-auto px-[4rem] py-[6rem] flex flex-col gap-[2.5rem]'>
                ...
                <div className='flex flex-row items-center justify-evenly w-[100%] h-auto'>
                    <Slide start={currentIndex} end={itemsPerPage}/>
                </div>

                <div className='flex flex-row items-center justify-center w-[100%] h-auto'>
                    <button onClick={()=> {handleClick(); setBtn1(true); setBtn2(false); setBtn3(false); setBtn4(false)}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn1? 'bg-transparent' : 'bg-[#35211e]'}  border-[#35211e] border-2`}></button>
                    <button onClick={()=> {handleClick(); setBtn1(false); setBtn2(true); setBtn3(false); setBtn4(false)}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn2? 'bg-transparent' : 'bg-[#35211e]'}  border-[#35211e] border-2`}></button>
                    <button onClick={()=> {handleClick(); setBtn1(false); setBtn2(false); setBtn3(true); setBtn4(false)}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn3? 'bg-transparent' : 'bg-[#35211e]'}  border-[#35211e] border-2`}></button>
                    <button onClick={()=> {handleClick(); setBtn1(false); setBtn2(false); setBtn3(false); setBtn4(true)}} className={`w-[0.8rem] h-[0.8rem] ml-1 rounded-full ${btn4? 'bg-transparent' : 'bg-[#35211e]'}  border-[#35211e] border-2`}></button>
                </div>
            </div>
        </>
    );
